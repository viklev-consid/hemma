...
3. Run: `dotnet new hemma-slice --module <Module> --name <Feature> --output src/Modules/<Module>/Hemma.Modules.<Module>/Features/<Feature>`
   (Adjust flags to match the actual template name in this repo — check `dotnet new list` if unsure.)
4. List the generated files with `ls src/Modules/<Module>/Hemma.Modules.<Module>/Features/<Feature>/` and verify all six pieces exist: Request, Response, Command, Handler, Validator, Endpoint.
