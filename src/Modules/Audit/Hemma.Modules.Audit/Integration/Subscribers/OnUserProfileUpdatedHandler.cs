...
[NonTransactional]
public sealed class OnUserProfileUpdatedHandler(AuditDbContext db, IClock clock)
{
    public async Task Handle(UserProfileUpdatedV1 @event, CancellationToken ct)
    {
        using var activity = AuditTelemetry.ActivitySource.StartActivity(nameof(OnUserProfileUpdatedHandler));
        AuditTelemetry.EventsProcessed.Add(1, new KeyValuePair<string, object?>("event", nameof(UserProfileUpdatedV1)));
