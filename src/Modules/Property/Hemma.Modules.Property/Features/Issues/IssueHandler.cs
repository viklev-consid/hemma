...
public sealed class IssueHandler(PropertyDbContext db, PropertyAuditPublisher audit, IClock clock)
{
    public async Task<ErrorOr<IssueResponse>> Handle(ReportIssueCommand cmd, CancellationToken ct)
    {
        ...
        return IssueResponse.FromIssue(issue.Value);
    }

    public async Task<ErrorOr<IssueResponse>> Handle(UpdateIssueCommand cmd, CancellationToken ct)
    {
    ...
    public async Task<ErrorOr<IssueResponse>> Handle(ChangeIssueStatusCommand cmd, CancellationToken ct)
    {
    ...
    public async Task<ErrorOr<Deleted>> Handle(DeleteIssueCommand cmd, CancellationToken ct)
    {
        var issue = await LoadIssueAsync(cmd.HouseholdId, cmd.IssueId, ct);
        if (issue is null)
        {
    ...
    public async Task<ErrorOr<IssueResponse>> Handle(LinkIssueToMaintenancePlanCommand cmd, CancellationToken ct)
    {
        var issue = await LoadIssueAsync(cmd.HouseholdId, cmd.IssueId, ct);
        if (issue is null)
        {
    ...
    public async Task<ErrorOr<IssueResponse>> Handle(LinkIssueToMaintenanceOccurrenceCommand cmd, CancellationToken ct)
    {
        var issue = await LoadIssueAsync(cmd.HouseholdId, cmd.IssueId, ct);
        if (issue is null)
        {
    ...
    public async Task<ErrorOr<IssueResponse>> Handle(UnlinkIssueCommand cmd, CancellationToken ct)
    {
        var issue = await LoadIssueAsync(cmd.HouseholdId, cmd.IssueId, ct);
        if (issue is null)
        {
    ...
    public async Task<ErrorOr<PromoteIssueToProjectResponse>> Handle(PromoteIssueToProjectCommand cmd, CancellationToken ct)
    {
        var issue = await LoadIssueAsync(cmd.HouseholdId, cmd.IssueId, ct);
        if (issue is null)
        {
        ...
        return new PromoteIssueToProjectResponse(IssueResponse.FromIssue(issue), ProjectResponse.FromProject(project.Value));
    }

    public async Task<ErrorOr<IssueResponse>> Handle(GetIssueQuery query, CancellationToken ct)
    {
        var issue = await LoadIssueAsync(query.HouseholdId, query.IssueId, ct, tracking: false);
        return issue is null ? PropertyErrors.IssueNotFound : IssueResponse.FromIssue(issue);
    }

    public async Task<ErrorOr<ListIssuesResponse>> Handle(ListIssuesQuery query, CancellationToken ct)
    {
        ...
        var items = await issues
            ...
            .Select(issue => IssueResponse.FromIssue(issue))
            .ToArrayAsync(ct);
