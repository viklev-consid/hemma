...
public sealed class AreasTagsOperations(PropertyDbContext db, PropertyAuditPublisher audit)
{
    ...
    public async Task<ErrorOr<PropertyAreaResponse>> UpdateAreaAsync(UpdateAreaCommand cmd, CancellationToken ct)
    {
    ...
    public async Task<ErrorOr<PropertyAreaResponse>> ArchiveAreaAsync(ArchiveAreaCommand cmd, CancellationToken ct)
    {
        var area = await db.Areas.SingleOrDefaultAsync(area => area.HouseholdId == cmd.HouseholdId && area.Id == new PropertyAreaId(cmd.AreaId), ct);
        if (area is null)
        {
    ...
    public async Task<ErrorOr<ListAreasResponse>> ListAreasAsync(ListAreasQuery query, CancellationToken ct)
    {
    ...
    public async Task<ErrorOr<PropertyTagResponse>> UpdateTagAsync(UpdateTagCommand cmd, CancellationToken ct)
    {
    ...
    public async Task<ErrorOr<PropertyTagResponse>> ArchiveTagAsync(ArchiveTagCommand cmd, CancellationToken ct)
    {
        var tag = await db.Tags.SingleOrDefaultAsync(tag => tag.HouseholdId == cmd.HouseholdId && tag.Id == new PropertyTagId(cmd.TagId), ct);
        if (tag is null)
        {
