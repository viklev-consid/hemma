...
Follow the repo-wide `/CLAUDE.md` and `/src/Modules/CLAUDE.md` rules. Economy is household-scoped and stores durable `HouseholdId` values as `Guid` columns in the `economy` schema. Do not query Households tables or add cross-schema foreign keys.
