...
This module owns the product catalog: creation, pricing, and availability. It is a simple reference module whose primary purpose is proving that a second module can exist alongside Users without boundary violations.
...
4. `Deactivate()` is irreversible - once a product is inactive, it cannot be reactivated. No re-activation method exists by design.

---

## Access control

- `ListProducts` and `GetProductById` are public.
- `CreateProduct` requires `CatalogPermissions.ProductsWrite` (`catalog.products.write`), granted to the `admin` role.

Permission constants live in `Hemma.Modules.Catalog.Contracts/Authorization/CatalogPermissions.cs`.
...
- `Money.Create(...)` works the same way. The EF value converter in `ProductConfiguration` calls `Sku.Create(...).Value`; this is safe only because EF reads well-formed data written by this module.
- Cross-module references must use only `Catalog.Contracts` events. Do not reference `Hemma.Modules.Catalog` internals from other modules.
