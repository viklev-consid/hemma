...
Prefer product modules publishing their own integration event and the Notifications module subscribing to it. For simple reusable cases, other modules may invoke `CreateNotificationCommand` from `Hemma.Modules.Notifications.Contracts`.
...
- `GET /v1/me/notifications/stream?clientId={clientId}` streams SSE events
- `GET|PUT /v1/me/notification-preferences`

SSE clients must provide a stable per-tab `clientId` query parameter. Treat it as an opaque URL-safe token, generated by the frontend and stored in `sessionStorage` rather than shared across browser tabs. When the same user reconnects with the same `clientId`, the backend replaces the previous stream and completes its writer. `MaxActiveStreamsPerUser` limits distinct active stream clients for the user, not reconnect attempts from the same client.
...
- Raw tokens arrive in `PasswordResetRequestedV1.RawToken`, `EmailChangeRequestedV1.RawToken`, `EmailConfirmationRequestedV1.RawToken`, user invitation events, and household invitation events. Embed them in email body links; never log them. Serilog destructuring masks known token property names, but defense-in-depth means not calling the log statement at all.
