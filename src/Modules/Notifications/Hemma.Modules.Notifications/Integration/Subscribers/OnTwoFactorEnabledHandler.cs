...
[NonTransactional]
public sealed class OnTwoFactorEnabledHandler(
    ...
    IEmailTemplateRenderer templateRenderer)
{
    ...
    {
        using var activity = NotificationsTelemetry.ActivitySource.StartActivity(nameof(OnTwoFactorEnabledHandler));
        NotificationsTelemetry.EventsProcessed.Add(1, new KeyValuePair<string, object?>("event", nameof(TwoFactorEnabledV1)));

        var log = NotificationLog.Create(@event.UserId, @event.Email, NotificationType.TwoFactorEnabled, TwoFactorEnabledTemplate.Subject, clock.UtcNow, @event.EventId);
