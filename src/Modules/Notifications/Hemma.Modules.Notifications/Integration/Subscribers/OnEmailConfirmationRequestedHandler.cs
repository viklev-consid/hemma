...
[NonTransactional]
public sealed class OnEmailConfirmationRequestedHandler(
    ...
    IEmailTemplateRenderer templateRenderer)
{
    ...
    {
        using var activity = NotificationsTelemetry.ActivitySource.StartActivity(nameof(OnEmailConfirmationRequestedHandler));
        NotificationsTelemetry.EventsProcessed.Add(1, new KeyValuePair<string, object?>("event", nameof(EmailConfirmationRequestedV1)));

        var log = NotificationLog.Create(
            @event.UserId,
            @event.Email,
            NotificationType.EmailConfirmationRequest,
            EmailConfirmationRequestTemplate.Subject,
            clock.UtcNow,
            @event.EventId);
